...
| ![fairy项目的文件资源系统中assets文件夹的内部目录](/images/2.png) | ![fairy项目的文件资源系统中assets/home文件夹的内部目录](/images/3.png) | ![fairy项目的文件资源系统中assets/home/res文件夹的内部目录](/images/6.png) | ![fairy项目的编辑器内资源库视图](/images/5.png) |
| :----------------------------------------------------------: | :----------------------------------------------------------: | :----------------------------------------------------------: | :---------------------------------------------: |
...
![fairy项目的导出内容](/images/image-20191030195617408.png)
...
![Fairy为Unity提供的SDK](/images/8.png)
...
![创建第一个fairy物件](/images/image-20191028144001261.png)
...
![StageEngine](/images/image-20191107163309097.png)
...
![UIContentScaler](/images/image-20191028145239101.png)
...
![PackageItem](/images/image-20191104100340406.png)
...
![PackageItem](/images/image-20191104100255757.png)
...
|                           type                           |                          objectType                          |
| :------------------------------------------------------: | :----------------------------------------------------------: |
| ![PackageItem.type](/images/image-20191101190720899.png) | ![PackageItem.objectType](/images/image-20191101190746365.png) |
| ![PackageItemType](/images/image-20191101190955691.png)  |      ![ObjectType](/images/image-20191101191058631.png)      |
...
![DisplayObject.gameObject](/images/image-20191025195401682.png)
...
![override GObject.CreateDisplayObject](/images/image-20191026113449931.png)
...
![GObject.displayObject](/images/image-20191025200048283.png)
...
![DisplayObjectEditor](/images/image-20191026144518747.png)
...
1. 获取到FairyGUI.DisplayObject所记录的坐标(a,b)，它是此DisplayObject相对于其父节点的坐标。
   ...
   ![FairyGUI.DisplayObject所记录的坐标](/images/image-20191027115857206.png)

2. 调整坐标值，得到其unity系本地坐标(a,-b,0)，便于后续利用UnityEngine提供的方法去操纵坐标。

   ![unity系本地坐标](/images/image-20191027120130070.png)
...
3. 根据此DisplayObject的cachedTransform属性（UnityEngine.Transform类型）， 使用UnityEngine.Transform.TransformPoint()方法，计算出其unity系世界坐标(d,e,f)。
   ...
   ![unity系世界坐标](/images/image-20191027123807892.png)
...
![EventListener构造函数](/images/14.png)

这使得每个事件接收器都对应一个事件分发器，一个事件桥。

![EventListener._bridge](/images/image-20191025111338305.png)
...
![EventBridge构造函数](/images/image-20191024204728335.png)

这使得每个事件桥都对应一个事件分发器。

![EventBridge.owner](/images/image-20191025111736660.png)

而一个事件分发器，应允许它使用多个事件桥，因此事件分发器中用一个词典记录了事件名-事件桥。

![EventDispatcher._dic](/images/image-20191024204050292.png)

#### 通过事件接收器注册事件

在业务层，比如给按钮注册点击事件，fairy使用的是**EventListener.Add**之类的方法。

![EventListener.Add](/images/image-20191025111917222.png)
...
![EventBridge.Add](/images/image-20191025114142835.png)
...
![EventBridge._callback1](/images/image-20191025114424356.png)

#### 通过事件分发器注册事件

在业务层，有时需要使用事件机制来传递参数，我们一般使用的是**EventDispatcher.AddEventListener**之类的方法。

![EventDispatcher.AddEventListener](/images/image-20191025130445516.png)
...
![EventDispatcher.DispatchEvent](/images/image-20191025125503784.png)

而事件分发器，会在自身记录的事件桥词典中，索引出这个事件桥，让事件桥去做相关逻辑操作。

![EventBridge.CallInternal](/images/image-20191025142435303.png)
...
![EventDispatcher.BubbleEvent](/images/image-20191025150106569.png)
...
![EventDispatcher.GetChainBridges](/images/image-20191025152609736.png)
...
![EventCallback0](/images/image-20191025143314318.png)

![EventCallback1](/images/image-20191025143346163.png)

EventCallback0不带参数，而EventCallback1带EventContext类型参数。

![EventContext](/images/image-20191025163607801.png)
...
![事件流概览](/images/image-20191025174106818.png)
...
![GTween](/images/image-20191029114220864.png)
...
![TweenManager](/images/image-20191028191338625.png)

#### 主要属性字段

TweenManager._activeTweens 记录了正在活跃的缓动

![TweenManager._activeTweens](/images/image-20191029093245950.png)

TweenManager._tweenerPool 为缓动的回收池

![TweenManager._tweenerPool](/images/image-20191029093301743.png)
...
![GTweener](/images/image-20191028194819824.png)

#### TweenValue

TweenValue用于缓动的帧的数据。

![TweenValue](/images/image-20191028195032375.png)

#### ITweenListener

ITweenListener是缓动侦听接口。

![ITweenListener](/images/image-20191028183227430.png)

一些有缓动需求的类实现了此接口。

![实现ITweenListener接口的类](/images/image-20191029172135379.png)

在GTweener中记录了一个ITweenListener类型的_listener字段。

![GTweener._listener](/images/image-20191029180738619.png)
...
缓动类型图解：![缓动类型图解](/images/image-20191029163457501.png)
...
![给组件创建动效](/images/image-20191029145743318.png)

#### 对元件的属性控制启用缓动

给一个组件创建了控制器之后，可以对其元件设置属性控制。

![元件的属性控制](/images/image-20191029150023870.png)
...
![Controller](/images/image-20191028180324311.png)

### 属性控制

在fairy元件的属性控制栏，可以设置其在此控制器下的显示控制。

![控制器的显示控制](/images/image-20191028155801183.png)

除此之外，fairy还提供了多种属性控制类型。

![控制器的属性控制](/images/image-20191028155921772.png)

每种属性控制，都是一种GearBase类型的子类。

![GearBase](/images/image-20191029211054678.png)
...
![修改控制器](/images/image-20191029210754358.png)
